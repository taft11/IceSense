...
export default function FulfillmentManagement() {
  ...
  const getFulfillmentMethod = (order) => String(order?.fulfillment_type || order?.fulfillmentMethod || order?.deliveryType || '').toLowerCase();

  const getPickupStatus = (order) => {
    const status = String(order?.pickupStatus || order?.status || '').toLowerCase().replace(/_/g, ' ');
    if (status === 'picked up' || status === 'released' || order?.pickedUpAt) return 'picked_up';
    if (status === 'ready for pickup' || status === 'ready' || order?.readyForPickup === true) return 'ready';
    return 'processing';
  };
  ...
  const getPickupCustomerPhone = (order) => order?.customerPhone || order?.contactNumber || userProfiles[order?.userId]?.contactNumber || 'No phone number';

  const getPickupItems = (order) => (Array.isArray(order?.items) ? order.items : []);
  ...
  const handlePickupStatusChange = async (order, nextStatus) => {
    if (!order?.id) return;

    try {
      setSavingOrderId(order.id);
      setError('');
      const update = nextStatus === 'ready'
        ? {
      ...
      setError('Unable to update the pickup order right now.');
    } finally {
      setSavingOrderId(null);
    }
  };
  ...
  const pickupOrders = orders
    ...
    .filter((order) => {
      ...
      return [order.id, order.orderId, order.customerName, order.customerEmail, getPickupCustomerPhone(order)]
        .filter(Boolean)
        .some((value) => String(value).toLowerCase().includes(query));
    })
    .sort((a, b) => (b.createdAt?.toMillis?.() || b.createdAt || 0) - (a.createdAt?.toMillis?.() || a.createdAt || 0));

  const pickupTotalPages = Math.max(1, Math.ceil(pickupOrders.length / ORDERS_PER_PAGE));
  const pickupSafePage = Math.min(currentPage, pickupTotalPages);
  const paginatedPickupOrders = pickupOrders.slice((pickupSafePage - 1) * ORDERS_PER_PAGE, pickupSafePage * ORDERS_PER_PAGE);
  ...
  const handlePickupFilterChange = (value) => {
  ...
  return (
    <div className="rounded-3xl border border-gray-100 bg-white p-8 shadow-sm">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Fulfillment Management</h2>
          <p className="mt-2 text-gray-600">Assign drivers for delivery orders or verify and release ice orders at the plant.</p>
        </div>
        <button
          type="button"
      ...
      <div className="mt-6 inline-flex rounded-full border border-slate-200 bg-slate-50 p-1">
        <button
          ...
          className={`inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold transition ${activeTab === 'deliveries' ? 'bg-[#4091c9] text-white shadow-sm' : 'text-slate-600 hover:bg-white'}`}
        >
          Deliveries
        </button>
        <button
          type="button"
          onClick={() => { setActiveTab('pickups'); setCurrentPage(1); }}
          className={`inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-semibold transition ${activeTab === 'pickups' ? 'bg-[#4091c9] text-white shadow-sm' : 'text-slate-600 hover:bg-white'}`}
        >
          Plant Pickups
        </button>
      </div>

      {activeTab === 'deliveries' && <div className="mt-4 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-3">
      ...
      {activeTab === 'pickups' && <div className="mt-4 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-3">
        <div className="inline-flex flex-wrap items-center gap-1 rounded-full border border-slate-200 bg-white p-1 shadow-sm">
          {Object.entries(PICKUP_FILTERS).map(([key, label]) => (
            <button
              key={key}
              type="button"
              onClick={() => handlePickupFilterChange(key)}
              className={`rounded-full px-3 py-2 text-sm font-semibold transition ${pickupFilter === key ? 'bg-[#4091c9] text-white shadow-sm' : 'bg-transparent text-slate-600 hover:bg-slate-100'}`}
            >
              {label}
            </button>
          ))}
        </div>
        <label className="ml-auto flex min-w-[260px] items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 shadow-sm">
          <Search className="h-4 w-4 text-slate-400" />
          <input
            type="search"
            value={pickupSearchTerm}
            onChange={(event) => handlePickupSearchChange(event.target.value)}
      ...
      {activeTab === 'pickups' ? (
        loading ? (
          <div className="mt-6 rounded-2xl border border-gray-200 bg-gray-50 p-6 text-sm text-gray-600">Loading pickup orders...</div>
        ) : paginatedPickupOrders.length === 0 ? (
          <div className="mt-6 rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-8 text-center text-gray-600">
            No pickup orders match the current filters.
          </div>
        ) : (
          <>
                ...
                <table className="min-w-full divide-y divide-gray-200 text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left font-semibold text-gray-700">Ordered Items</th>
                      <th className="px-4 py-3 text-left font-semibold text-gray-700">Pickup Window</th>
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {paginatedPickupOrders.map((order) => {
                      ...
                      const statusLabel = PICKUP_FILTERS[pickupStatus];
                      return (
                        <tr key={order.id} className="align-top">
                          <td className="px-4 py-4">
                            <p className="font-semibold text-gray-800">#{(order.id || '').slice(0, 8).toUpperCase()}</p>
                            <p className="mt-1 text-xs text-gray-500">{formatOrderTimestamp(order.createdAt)}</p>
                          </td>
                          <td className="px-4 py-4">
                            <p className="font-semibold text-gray-800">{order.customerName || 'Unknown customer'}</p>
                            <p className="mt-1 text-xs text-gray-500">{getPickupCustomerPhone(order)}</p>
                          </td>
                          <td className="px-4 py-4">
                            <div className="min-w-[190px] space-y-1 text-gray-700">
                              {getPickupItems(order).map((item, index) => (
                                <p key={`${order.id}-pickup-item-${index}`}>
                                  {item.weightKg ? `${item.weightKg}kg ` : ''}{item.name || item.productName || 'Ice item'} <strong>× {item.quantity || 0}</strong>
                                </p>
                              ))}
                            </div>
                          </td>
                          <td className="px-4 py-4 text-gray-600">
                            <p>{order.deliveryDate || 'Date not selected'}</p>
                            <p className="mt-1 text-xs text-gray-500">{order.deliveryTimeSlot || order.deliverySlot || 'Time not selected'}</p>
                          </td>
                          <td className="px-4 py-4">
                            <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${pickupStatus === 'picked_up' ? 'bg-green-100 text-green-700' : pickupStatus === 'ready' ? 'bg-purple-100 text-purple-700' : 'bg-blue-100 text-blue-700'}`}>
                              {statusLabel}
                            </span>
                          </td>
                          <td className="px-4 py-4 text-right">
                            {pickupStatus === 'processing' && (
                              <button type="button" onClick={() => handlePickupStatusChange(order, 'ready')} disabled={savingOrderId === order.id} className="whitespace-nowrap rounded-xl bg-[#4091c9] px-3 py-2 text-xs font-semibold text-white transition hover:bg-[#2d75aa] disabled:cursor-not-allowed disabled:opacity-50">
                                Mark Ready for Pickup
                              </button>
                            )}
                            {pickupStatus === 'ready' && (
                              <button type="button" onClick={() => handlePickupStatusChange(order, 'picked_up')} disabled={savingOrderId === order.id} className="inline-flex items-center gap-1 whitespace-nowrap rounded-xl bg-green-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-green-700 disabled:cursor-not-allowed disabled:opacity-50">
                                Confirm Release
                              </button>
                            )}
                            {pickupStatus === 'picked_up' && <span className="text-xs font-semibold text-slate-500">Released</span>}
                          </td>
                        </tr>
                      );
                    })}
            ...
            <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-gray-500">Showing {Math.min((pickupSafePage - 1) * ORDERS_PER_PAGE + 1, pickupOrders.length)}-{Math.min(pickupSafePage * ORDERS_PER_PAGE, pickupOrders.length)} of {pickupOrders.length} pickup orders</p>
              <div className="flex items-center gap-2">
                <button type="button" onClick={() => setCurrentPage((page) => Math.max(1, page - 1))} disabled={pickupSafePage === 1} className="rounded-lg border border-gray-200 px-3 py-2 text-sm font-semibold text-gray-600 disabled:opacity-50">Prev</button>
                <span className="text-sm font-semibold text-gray-700">Page {pickupSafePage} of {pickupTotalPages}</span>
                <button type="button" onClick={() => setCurrentPage((page) => Math.min(pickupTotalPages, page + 1))} disabled={pickupSafePage === pickupTotalPages} className="rounded-lg border border-gray-200 px-3 py-2 text-sm font-semibold text-gray-600 disabled:opacity-50">Next</button>
              </div>
            </div>
          </>
        )
